...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[mapbox, below=of bfs] (tile) {TilemapGenerator curriculum};
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3.4cm}|p{4.5cm}|p{3.5cm}|c|}
        ...
        \hline
        Curriculum learning & Phần thưởng thưa và trễ & §2.7 & MT2 \\
        \hline
...
Theo nền tảng ở Chương~1, môi trường chiến đấu trong khóa luận được mô hình hóa như một Quy trình Quyết định Markov Quan sát một phần (Partially Observable Markov Decision Process -- POMDP) với bộ thành phần $(S, A, O, P, R, \Omega, \gamma)$. Cách mô hình hóa này phù hợp hơn MDP đầy đủ vì tác nhân không được truy cập trực tiếp toàn bộ trạng thái của phòng chơi. Tương tự người chơi, CombatAgent chỉ nhận thông tin trong phạm vi tầm nhìn VisionRadius=20 và còn bị ràng buộc bởi yêu cầu đường ngắm RequireLineOfSightToTarget=\textit{true}. Do đó, kẻ địch hoặc đạn nằm ngoài tầm nhìn, hoặc bị tường che khuất, không được đưa vào dữ liệu nhận thức của game. Đây là đặc tính của môi trường Roguelike chiến đấu, không phải một giới hạn tùy ý được áp lên thuật toán học.

Trong bối cảnh của đề tài, các thành phần POMDP được hiểu như sau:
\begin{itemize}
    \item $S$ -- \textbf{trạng thái thật của môi trường}: toàn bộ tilemap, vị trí và nội trạng của người chơi và kẻ địch (máu, hồi chiêu, vận tốc), vị trí và vận tốc của các viên đạn đang bay, cùng bộ đếm thời gian của lượt chơi. Trạng thái này tồn tại trong Unity nhưng tác nhân \emph{không quan sát trực tiếp}.
    \item $A$ -- \textbf{không gian hành động}: không gian rời rạc nhiều nhánh $(3,3,3,9)$, tương ứng với di chuyển, chiến đấu và hướng tấn công (đặc tả ở §2.4).
    \item $O$ -- \textbf{không gian quan sát}: vector đặc trưng mà CombatAgent nhận được tại mỗi bước. Cấu hình cơ sở dùng 207 chiều; run59 mở rộng thành 210 chiều bằng đặc trưng đường đi, còn run62 dùng 432 chiều với lưới cục bộ $15\times15$. Cấu trúc quan sát 207 chiều được trình bày ở §2.3.
    \item $P(s_{t+1} \mid s_t, a_t)$ -- \textbf{hàm chuyển trạng thái}: được quyết định bởi logic gameplay, vật lý 2D, hành vi kẻ địch, đạn và cơ chế sinh bản đồ trong Unity.
    \item $\Omega(o_t \mid s_t, a_{t-1})$ -- \textbf{hàm phát quan sát}: ánh xạ trạng thái thật $s_t$ thành quan sát $o_t$ thông qua các bộ trích đặc trưng của CombatAgent, như sắp xếp kẻ địch/đạn theo khoảng cách và dò tia ray tới tường.
    \item $R$ -- \textbf{hàm phần thưởng}: kết hợp tín hiệu kết quả thưa với các tín hiệu định hình dày hơn nhằm hỗ trợ học trong giai đoạn đầu (đặc tả ở §2.5).
    \item $\gamma = 0.99$ -- \textbf{hệ số chiết khấu}: giá trị dùng trong cấu hình PPO của các thí nghiệm chính.
\end{itemize}
...
Cách tiếp cận được chọn là tách bốn nhóm thông tin (người chơi, kẻ địch, đạn, tường) thành các đoạn có kích thước cố định, dùng tia ray cho tường thay vì lưới chiếm chỗ để giảm chiều, và đệm bằng 0 khi số kẻ địch/đạn thay đổi giữa các lượt chơi.
...
CombatAgent sử dụng không gian hành động rời rạc nhiều nhánh gồm bốn nhánh với kích thước $(3,3,3,9)$. Cách biểu diễn này cho phép tách riêng các quyết định điều khiển quan trọng trong mỗi bước thời gian: di chuyển theo trục X, di chuyển theo trục Y, lệnh chiến đấu và hướng tấn công. Nhờ đó, tác nhân không chỉ chọn một hành vi tổng quát, mà có thể phối hợp đồng thời giữa di chuyển, tấn công, lướt và hướng tấn công trong môi trường chiến đấu thời gian thực.
...
Để giải quyết thách thức bản đồ thay đổi qua từng lượt chơi (MT2), tác nhân cần được huấn luyện trên nhiều bản đồ thay vì một bố cục cố định. Tuy nhiên, gọi trực tiếp generator Python từ Unity ở mỗi lượt chơi tạo độ trễ và rủi ro lỗi tích hợp; mặt khác sinh bản đồ không kiểm tra có thể tạo ra bố cục nơi người chơi và kẻ địch nằm trên hai vùng tách rời.
...
Bảng~\ref{tab:difficulty_spawn_policy} tóm tắt cách chọn bản đồ và vị trí sinh trong từng mức độ khó. Trong các lần chạy dùng curriculum learning, tham số môi trường \emph{difficulty} do ML-Agents truyền vào quyết định cả thư mục bản đồ lẫn chế độ sinh. Với \emph{difficulty}=0, 1 và 2, hệ thống dùng vị trí sinh cố định: trên mỗi bản đồ, vị trí người chơi được chọn theo một quy tắc xác định và được lưu tạm, còn các vị trí kẻ địch được chọn từ các ô sàn có thể đi tới từ người chơi, trải theo các mức khoảng cách trung bình để tránh việc kẻ địch luôn xuất hiện quá gần hoặc quá xa. Với \emph{difficulty}=3, hệ thống vẫn dùng tập Hard nhưng chuyển sang vị trí sinh ngẫu nhiên: người chơi và kẻ địch được lấy ngẫu nhiên từ các ô sàn hợp lệ sau mỗi lượt chơi. Trong cấu hình huấn luyện chính, số kẻ địch sinh ra mỗi lượt chơi được giới hạn bởi \emph{randomEnemySpawnCount}=2; nếu có nhiều prefab enemy, prefab cụ thể được lấy từ mảng \emph{enemyPrefabs}.
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|p{2.3cm}|p{2.5cm}|p{4.0cm}|p{4.0cm}|}
        ...
        \hline
        3 & Hard & Random & Chọn ngẫu nhiên một ô sàn hợp lệ ở mỗi lượt chơi. & Chọn ngẫu nhiên tối đa 2 ô sàn hợp lệ có thể đi tới từ người chơi, nhằm tăng khả năng khái quát hóa với vị trí sinh mới. \\
        \hline
    \end{tabular}
    }
\end{table}

Như vậy, độ khó không chỉ tăng vì bản đồ chuyển từ Easy sang Medium và Hard, mà còn vì phương sai vị trí sinh được mở rộng ở giai đoạn Hard Random. Cách tách này giúp tác nhân học trước các kỹ năng cơ bản như tiếp cận, né đạn và căn hướng tấn công trong môi trường ít nhiễu hơn, rồi mới phải thích nghi với vị trí sinh thay đổi mạnh.

\section{Khung học theo chương trình}

Khi tác nhân khởi tạo ngẫu nhiên và phải học ngay trên nhóm Hard, phần thưởng xuất hiện rất thưa; xác suất tình cờ dọn được phòng gần như bằng 0, nên PPO không có tín hiệu học hiệu quả. Để phục vụ MT2, hệ thống cần một cơ chế tạo tín hiệu học sớm mà không phải sửa thuật toán PPO.

Curriculum learning tổ chức bài toán thành chuỗi từ dễ đến khó: bắt đầu ở Easy (mật độ tường thấp, không gian rộng) nơi tác nhân thường xuyên hoàn thành phòng, dùng EMA của phần thưởng làm điều kiện chuyển sang Medium rồi Hard. ML-Agents cung cấp cơ chế truyền tham số \emph{difficulty} từ trainer xuống môi trường, gắn trực tiếp với TilemapGenerator.

Curriculum learning được dùng để giảm độ khó khám phá ban đầu \cite{Bengio2009}. Thay vì đưa tác nhân ngay vào toàn bộ nhóm Hard, hệ thống bắt đầu từ bản đồ dễ hơn, chuyển sang Medium khi phần thưởng làm mượt vượt ngưỡng, rồi mới chuyển sang nhóm bản đồ Hard. Tham số độ khó được truyền qua Academy.Instance.EnvironmentParameters với tên difficulty.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[>=stealth,
        ...
        \node[level] (easy) at (0,0) {Mức 0\\Easy};
        \node[level] (medium) at (5.0,0) {Mức 1\\Medium};
        \node[level] (hard) at (10.0,0) {Mức 2\\Hard};
        \node[level, minimum width=3.2cm] (random) at (10.0,-1.8) {Mức 3\\Hard Random};
    ...
    \end{tikzpicture}
    \caption{Cấu trúc curriculum 3 bậc và biến thể 4 bậc chuyển từ vị trí sinh cố định sang ngẫu nhiên}
    \label{fig:curriculum_structure}
\end{figure}

Trong run46, curriculum gồm 3 mức độ khó: Easy, Medium và Hard. Trong run54, curriculum được mở rộng thành 4 mức: \emph{Easy Fixed}, \emph{Medium Fixed}, \emph{Hard Fixed} và \emph{Hard Random}. Biến thể 4 bậc kiểm tra giả thuyết rằng việc giảm phương sai vị trí sinh ở giai đoạn đầu có thể giúp PPO ổn định hơn trước khi chuyển sang phân phối khó hơn.

Cần phân biệt hai lớp tổ chức độ khó. Lớp thứ nhất là chia tập bản đồ PCGNN thành các thư mục Easy, Medium và Hard dựa trên chỉ số hình học. Lớp thứ hai là cơ chế chuyển mức độ khó trong ML-Agents: trainer thay đổi tham số \emph{difficulty} khi phần thưởng làm mượt vượt ngưỡng, từ đó TilemapGenerator chọn thư mục bản đồ và chế độ sinh tương ứng.

\section{Kiến trúc mạng và cấu hình PPO}

Để phục vụ MT1 và MT3, kiến trúc mạng và các siêu tham số PPO cần đủ ổn định để các lần chạy thí nghiệm loại trừ chỉ khác nhau ở \emph{đúng một biến} (curriculum, ICM, RND, LSTM\dots). Nếu không, chênh lệch kết quả có thể bị quy nhầm cho kỹ thuật được kiểm tra trong khi nguyên nhân thực sự nằm ở siêu tham số.

Cấu hình cơ sở vì vậy cố định MLP actor-critic hai lớp 256 đơn vị (hidden\_units=256, num\_layers=2) và giữ nguyên các siêu tham số PPO chính giữa các lần chạy thí nghiệm loại trừ; chỉ thay đổi biến cần kiểm tra. Các lần chạy LSTM bổ sung bộ nhớ nhưng giữ phần MLP nền.
...
Một số lần chạy thí nghiệm loại trừ bổ sung bộ nhớ LSTM với memory\_size=256 và sequence\_length=64 để kiểm tra giả thuyết rằng thông tin lịch sử có thể giúp tác nhân xử lý đạn, hồi chiêu và tình huống khuất tầm nhìn. Kết quả định lượng của các lựa chọn kiến trúc này được trình bày ở Chương~4.

Chính sách dùng MLP actor-critic (hidden\_units=256, num\_layers=2, normalize=true). Actor sinh phân phối xác suất cho 4 nhánh hành động; Critic ước lượng $V(s)$ làm mốc nền cho GAE. Các lần chạy LSTM bổ sung memory\_size=256, sequence\_length=64. Các siêu tham số PPO chính (learning\_rate=$3\times10^{-4}$, batch\_size=1024, buffer\_size=10240, num\_epoch=3, $\gamma$=0.99, $\lambda$=0.95, $\epsilon$=0.2, $\beta$=$5\times10^{-3}$) được giữ thống nhất giữa các lần chạy thí nghiệm loại trừ run34--run54 để cô lập tác động của từng kỹ thuật. Chi tiết các thay đổi trong run62 được trình bày ở Chương~3.

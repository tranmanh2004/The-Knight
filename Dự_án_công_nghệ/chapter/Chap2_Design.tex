...
Cần phân biệt hai lớp tổ chức độ khó. Lớp thứ nhất là chia tập bản đồ PCGNN thành các folder Easy, Medium và Hard dựa trên chỉ số hình học. Lớp thứ hai là lịch chuyển lesson trong ML-Agents: trainer thay đổi tham số \texttt{difficulty} khi reward làm mượt vượt ngưỡng, từ đó TilemapGenerator chọn folder bản đồ và chế độ spawn tương ứng.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{4.0cm}|c|p{7.2cm}|}
        ...
        \hline
        \texttt{learning\_rate}      & $3{\times}10^{-4}$ & Tốc độ học của Adam optimizer; là giá trị mặc định khuyến nghị của ML-Agents cho PPO. \\
        \hline
        \texttt{learning\_rate\_schedule} & linear & Giảm tuyến tính learning rate xuống 0 ở cuối quá trình huấn luyện để ổn định cập nhật giai đoạn cuối. \\
        \hline
        \texttt{batch\_size}         & 1024  & Kích thước minibatch khi cập nhật policy. \\
        \hline
        \texttt{buffer\_size}        & 10240 & Số bước trải nghiệm thu thập trước mỗi lần cập nhật PPO. \\
        \hline
        \texttt{num\_epoch}          & 3     & Số lần lặp qua buffer trong một lần cập nhật. \\
        \hline
        $\gamma$ (\texttt{gamma})    & 0.99  & Hệ số chiết khấu cho phần thưởng tương lai. \\
        \hline
        $\lambda$ (\texttt{lambd})   & 0.95  & Tham số GAE cân bằng độ lệch và phương sai (Phương trình~\ref{eq:gae_formula}). \\
        \hline
        $\epsilon$ (\texttt{epsilon})& 0.2   & Biên độ cắt PPO của tỷ lệ xác suất $r_t(\theta)$ (Phương trình~\ref{eq:ppo_clip}). \\
        \hline
        $\beta$ (\texttt{beta})      & $5{\times}10^{-3}$ & Hệ số entropy bonus, được giảm tuyến tính để khuyến khích khám phá ban đầu. \\
        \hline
        \texttt{time\_horizon}       & 64    & Số bước trước khi bootstrap value estimate khi episode chưa kết thúc. \\
        \hline
        \texttt{hidden\_units}       & 256   & Số neuron của mỗi lớp ẩn trong shared MLP backbone. \\
        \hline
        \texttt{num\_layers}         & 2     & Số lớp ẩn của shared MLP backbone. \\
        \hline
        \texttt{normalize}           & true  & Chuẩn hóa observation theo running mean/std để ổn định gradient. \\
        \hline
        \texttt{memory\_size}        & 256   & Kích thước trạng thái ẩn LSTM (chỉ áp dụng cho run42, run47). \\
        \hline
        \texttt{sequence\_length}    & 64    & Độ dài chuỗi quan sát đưa vào LSTM (chỉ áp dụng cho run42, run47). \\
        \hline
        \texttt{max\_steps}          & 1M / 3M & Tổng bước huấn luyện: 1M cho các run baseline (run34--42), 3M cho các run curriculum (run46+). \\
        \hline
    \end{tabular}
\end{table}

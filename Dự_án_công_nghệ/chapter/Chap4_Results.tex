...
Chương này trình bày kết quả thực nghiệm trên CombatAgent. Khác với kết quả ban đầu chỉ gồm một run ngắn 200 nghìn bước, bộ thí nghiệm cuối cùng gồm nhiều run từ 1 triệu đến 3 triệu bước, tập trung vào các yếu tố: phần thưởng nội tại, LSTM, học theo chương trình, giảm phương sai vị trí sinh và thiết kế lại phần thưởng. Kết quả chính cho thấy học theo chương trình là yếu tố tạo ra cải thiện lớn nhất; ICM chỉ tạo khác biệt rõ khi được ghép sau curriculum, còn các kỹ thuật còn lại chủ yếu dao động quanh cùng một mức trần hiệu năng.

\section{Chỉ số đánh giá}

Các scalar được trích từ event file của TensorBoard bằng script scripts/extract\_run\_metrics.py. Dữ liệu sau trích xuất được lưu trong data/run\_metrics.csv và data/rolling\_reward\_200k.csv. Bảng~\ref{tab:tensorboard_metrics} trình bày các chỉ số được dùng trong phần phân tích.
...
\begin{table}[H]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{3.8cm}|>{\raggedright\arraybackslash}p{6.0cm}|}
        \hline
        ...
        \hline
        Reward trung bình trượt & rolling\_reward\_200k.csv & Reward được làm mượt theo cửa sổ 200 nghìn bước, dùng để vẽ biểu đồ xu hướng học thay vì chỉ nhìn một điểm cuối. \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \caption{Các chỉ số hành vi chính trích từ data/run\_metrics.csv}
...
\section{Baseline PPO và các intrinsic reward}

Run34 là baseline PPO không curriculum. Sau 1 triệu bước, tail-50 cumulative reward đạt 2.073, tỷ lệ clear tail-50 đạt 0.129 và peak clear đạt 0.50. Agent học được các hành vi cơ bản như tiếp cận và tấn công, nhưng chưa duy trì ổn định khả năng dọn phòng trên phân phối map khó.

Hai biến thể intrinsic motivation gồm ICM (run35) và RND (run36). ICM đạt reward tail-50 là 2.416, RND đạt 2.487. Mức tăng so với baseline tồn tại nhưng nhỏ, không tạo thay đổi bản chất. Điều này gợi ý rằng trong môi trường này, vấn đề chính không chỉ là thiếu tín hiệu khám phá, mà là độ khó ban đầu của phân phối map và nhiệm vụ combat.

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=1.05cm,y=0.78cm, every node/.style={font=\scriptsize}]
        \draw[->] (0,0) -- (5.7,0) node[right]{step};
        \draw[->] (0,0) -- (0,4.2) node[above]{reward};
        \foreach \y in {1,2,3,4} \draw[gray!30] (0,\y) -- (5.3,\y) node[left, black]{\y};
        \foreach \x/\label in {0.5/100k,1.5/300k,2.5/500k,3.5/700k,5.0/1M}
            \draw (\x,0) -- (\x,-0.08) node[below]{\label};
        \draw[thick, gray] plot coordinates {(0.5,2.82) (1.5,3.17) (2.5,2.14) (3.5,1.84) (5.0,2.11)};
        \draw[thick, blue] plot coordinates {(0.5,3.29) (1.5,1.81) (2.5,2.43) (3.5,2.33) (5.0,2.39)};
        \draw[thick, green!50!black] plot coordinates {(0.5,2.90) (1.5,2.60) (2.5,2.62) (3.5,2.42) (5.0,2.62)};
        \node[gray, anchor=west] at (5.35,2.11){PPO};
        \node[blue, anchor=west] at (5.35,2.39){ICM};
        \node[green!50!black, anchor=west] at (5.35,2.62){RND};
    \end{tikzpicture}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=1.1cm,y=0.65cm, every node/.style={font=\scriptsize}]
...
Run42 thêm LSTM với bộ nhớ kích thước 256 và chuỗi quan sát dài 64 bước. Khác với ICM/RND, LSTM có xu hướng học chậm nhưng tăng dần về cuối. Rolling reward 200k bước tăng từ 1.64 ở mốc 100k lên 2.71 ở mốc 1M. Điều này phù hợp với giả thuyết rằng bộ nhớ có ích cho hồi chiêu, đạn và trạng thái khuất tầm nhìn, nhưng lợi ích chưa đủ lớn để tự tạo cải thiện đột phá.

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=1.05cm,y=0.9cm, every node/.style={font=\scriptsize}]
        \draw[->] (0,0) -- (5.7,0) node[right]{step};
...
\section{Curriculum learning: kết quả đột phá}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=0.9cm,y=1cm, every node/.style={font=\scriptsize}]
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=0.8cm,y=0.65cm, every node/.style={font=\scriptsize}]
        \draw[->] (0,0) -- (9.2,0) node[right]{step};
        \draw[->] (0,0) -- (0,6.2) node[above]{reward};
        \foreach \y in {1,2,3,4,5,6} \draw[gray!25] (0,\y) -- (8.8,\y) node[left, black]{\y};
        \foreach \x/\label in {0.4/100k,1.4/300k,2.4/500k,3.4/700k,4.4/1M,5.4/1.5M,6.4/2M,7.4/2.5M,8.4/3M}
            \draw (\x,0) -- (\x,-0.08) node[below]{\label};
        \draw[thick, blue] plot coordinates {(0.4,5.59) (1.4,4.15) (2.4,3.60) (3.4,3.11) (4.4,3.41) (5.4,4.03) (6.4,3.53) (7.4,3.92) (8.4,4.12)};
    \end{tikzpicture}
    \caption{Rolling reward 200k bước của run46 sau khi áp dụng curriculum}
    \label{fig:run46_rolling_reward}
\end{figure}

Kết quả này là bằng chứng mạnh nhất của khóa luận: curriculum không chỉ tăng reward trung bình mà còn tăng các chỉ số hành vi. Tail-50 của run46 đạt 0.994 kills/episode và cleared tail-50 đạt 0.267, cao hơn rõ rệt so với baseline. Về mặt cơ chế, curriculum tạo "giàn giáo gradient": ở Easy, agent gặp các episode thành công đủ thường xuyên để học chuỗi tiếp cận -- ngắm -- tấn công; khi sang Medium và Hard, policy đã có kỹ năng nền thay vì khám phá từ gần như ngẫu nhiên.

\section{Stacking test: LSTM và ICM trên nền curriculum}
...
Run50 thêm ICM vào curriculum và đã được chạy đủ 3.00M bước. Reward mean trên vùng Hard đạt 4.369, cao hơn run46 là 3.843; tail-50 cuối run đạt 4.707. Các chỉ số hành vi cũng tăng nhẹ: kills đạt 1.016, cleared tail-50 đạt 0.273 và useful dash rate đạt 0.334. Tuy nhiên inverse loss vẫn quanh 4.337, chỉ giảm nhẹ so với run35 và vẫn gần vùng cao so với entropy ngẫu nhiên lý thuyết. Vì vậy, kết quả mới cho thấy ICM có cộng hưởng dương khi đặt sau curriculum, nhưng chưa giải quyết triệt để vấn đề representation của action space nhiều nhánh.
...
\section{Giảm phương sai môi trường bằng fixed-to-random spawn}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=0.85cm,y=0.9cm, every node/.style={font=\scriptsize}]
        \draw[->] (0,0) -- (9.2,0) node[right]{step};
        \draw[thick] (0,0.8) -- (1.4,0.8);
        \draw[thick] (1.4,1.5) -- (3.0,1.5);
        \draw[thick] (3.0,2.2) -- (4.8,2.2);
        \draw[thick] (4.8,2.9) -- (8.8,2.9);
        \draw[dashed] (1.4,0) -- (1.4,1.8);
        \draw[dashed] (3.0,0) -- (3.0,2.5);
        \draw[dashed] (4.8,0) -- (4.8,3.2);
        \node at (0.7,1.05){Easy F};
        \node at (2.2,1.75){Medium F};
        \node at (3.9,2.45){Hard F};
        \node at (6.7,3.15){Hard R};
        \node[below] at (0,0){10k};
        \node[below] at (1.4,0){60k};
        \node[below] at (3.0,0){130k};
        \node[below] at (4.8,0){280k};
        \node[below] at (8.8,0){3M};
    \end{tikzpicture}
...
Fixed spawn giúp giảm phương sai ở giai đoạn đầu, nhưng khi chuyển sang Hard Random thì agent vẫn phải xử lý cùng phân phối khó. Vì vậy, run54 chỉ cải thiện nhẹ ở hành vi combat và không thay đổi kết luận tổng quát: curriculum là yếu tố chính, còn fixed-to-random là tinh chỉnh môi trường.

\section{Reward v2: tăng outcome reward nhưng đổi thang đo}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=1cm,y=4.2cm, every node/.style={font=\scriptsize}]
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=0.75cm,y=1.35cm, every node/.style={font=\scriptsize}]
        \draw[->] (0,4.0) -- (10.2,4.0) node[right]{run};
        \draw[->] (0,4.0) -- (0,5.75) node[above]{entropy};
        \draw[dashed, red] (0,5.49) -- (9.8,5.49);
        \node[red, anchor=west] at (9.85,5.49){5.49};
        \foreach \y/\label in {4.2/4.2,4.4/4.4,4.6/4.6,4.8/4.8,5.0/5.0,5.2/5.2}
            \draw[gray!20] (0,\y) -- (9.8,\y) node[left, black]{\label};
        \fill[blue!55] (0.7,4.0) rectangle (1.2,4.408); \node[below] at (0.95,4.0){34};
        \fill[blue!55] (1.7,4.0) rectangle (2.2,4.407); \node[below] at (1.95,4.0){35};
        \fill[blue!55] (2.7,4.0) rectangle (3.2,4.406); \node[below] at (2.95,4.0){36};
        \fill[blue!55] (3.7,4.0) rectangle (4.2,4.405); \node[below] at (3.95,4.0){42};
        \fill[blue!55] (4.7,4.0) rectangle (5.2,4.409); \node[below] at (4.95,4.0){46};
        \fill[blue!55] (5.7,4.0) rectangle (6.2,4.405); \node[below] at (5.95,4.0){47};
        \fill[blue!55] (6.7,4.0) rectangle (7.2,4.412); \node[below] at (6.95,4.0){50};
        \fill[blue!55] (7.7,4.0) rectangle (8.2,4.407); \node[below] at (7.95,4.0){54};
        \fill[blue!55] (8.7,4.0) rectangle (9.2,4.408); \node[below] at (8.95,4.0){55};
    \end{tikzpicture}
    \caption{Entropy tail-50 của các run chính đều nằm quanh 4.40 nat}
...
\begin{table}[H]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{1.15cm}|>{\raggedright\arraybackslash}p{2.65cm}|>{\centering\arraybackslash}p{1.05cm}|>{\centering\arraybackslash}p{1.95cm}|>{\centering\arraybackslash}p{1.35cm}|>{\raggedright\arraybackslash}p{4.65cm}|}
        ...
        \hline
        run36 & PPO + RND & 1.0M & 2.487 & 0.796 & Tốt hơn ICM nhẹ, nhưng chưa tạo đột phá. \\
        \hline

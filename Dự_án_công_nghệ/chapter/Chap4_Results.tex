...
Fixed spawn giúp giảm phương sai ở giai đoạn đầu, nhưng khi chuyển sang Hard Random thì agent vẫn phải xử lý cùng phân phối khó. Vì vậy, run54 chỉ cải thiện nhẹ ở hành vi combat và không thay đổi kết luận tổng quát: curriculum là yếu tố chính, còn fixed-to-random chỉ là một biến thể của thiết kế môi trường, đóng góp không đáng kể so với chính curriculum.
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[x=1.0cm, y=3.5cm, every node/.style={font=\scriptsize}]
...
\begin{figure}[H]
\centering
\begin{tikzpicture}[x=1.3cm, y=1.1cm, every node/.style={font=\scriptsize}]
  % Axes
  \draw[->] (0,0) -- (9.5,0) node[right]{Run};
  \draw[->] (0,0) -- (0,5.3) node[above]{Reward mean};
  % Grid

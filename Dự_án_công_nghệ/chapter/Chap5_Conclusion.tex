...
\begin{table}[H]
    ...
    \begin{tabular}{|p{4.5cm}|c|p{7cm}|}
        ...
        \hline
        Curriculum run46 & 3.843 Hard mean & Cải thiện khoảng 85\% so với baseline. \\
        \hline
        Curriculum + ICM run50 & 4.369 Hard mean & Cải thiện thêm trên nền curriculum, nhưng inverse loss vẫn cao. \\
        \hline
        Fixed-to-random run54 & 3.925 Hard mean & Tương đương run46, cho thấy fixed spawn chỉ là biến thể nhỏ của môi trường. \\
        \hline
        Reward v2 run55 & 0.800 cleared peak & Tăng tỷ lệ clear peak, nhưng dùng thang reward khác. \\
        \hline
...
Các thí nghiệm ICM, RND và LSTM đem lại thông tin bổ sung nhưng không thay đổi kết luận chính. Khi chạy độc lập, ICM có dấu hiệu inverse model học yếu trên action space nhiều nhánh; RND cải thiện nhẹ hơn ICM nhưng không vượt trần; LSTM có xu hướng học chậm và tăng dần nhưng không tạo breakthrough. Khi ghép với curriculum, LSTM không tạo cộng hưởng rõ, còn ICM trong run50 giúp tăng reward và kills nhưng inverse loss vẫn cao.
...
Đóng góp thứ hai là kết quả thực nghiệm về curriculum learning trong môi trường procedural combat. Các số liệu cho thấy việc đưa agent qua chuỗi Easy -- Medium -- Hard giúp policy học được kỹ năng nền trước khi đối mặt với phân phối khó, từ đó tạo cải thiện lớn hơn nhiều so với intrinsic reward hoặc thay đổi kiến trúc memory khi các kỹ thuật này chạy độc lập.
...
Hạn chế lớn nhất của khóa luận là mỗi run mới chỉ được chạy một seed. Vì vậy, các kết luận về chênh lệch nhỏ cần được xem là xu hướng thực nghiệm, chưa phải kết quả thống kê có error bar. Các phát hiện lớn như curriculum tăng khoảng 85\% có độ tin cậy cao hơn vì biên chênh lệch lớn, nhưng vẫn cần chạy nhiều seed để xác nhận.

Hạn chế thứ hai là PCGNN hiện được dùng theo hướng offline và có bước con người tuyển chọn. Cách này giúp ổn định huấn luyện và giảm lỗi tích hợp, nhưng chưa phải adaptive PCG theo thời gian thực, nơi độ khó map thay đổi trực tiếp dựa trên năng lực hiện tại của agent.
...
Hạn chế cuối cùng là entropy plateau chưa được giải quyết trong phạm vi khóa luận. Các thí nghiệm đã loại trừ một số nguyên nhân trực tiếp như reward scale hoặc curiosity signal, nhưng chưa kiểm tra sâu các hướng như factorized policy khác, action abstraction hoặc imitation learning bootstrap.

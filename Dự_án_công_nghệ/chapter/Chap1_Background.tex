...
\node[box, fill=green!15]  (cr) at (8.0,-1.1)  {Critic\\$V_\phi(s)$};
...
\draw[->] (cr) -- (v);
\end{tikzpicture}
\caption{Kiến trúc Actor-Critic: Actor và Critic chia sẻ các lớp nền chung, sau đó phân nhánh -- Actor sinh phân phối hành động $\pi_\theta(a|s)$, Critic ước lượng giá trị $V_\phi(s)$ làm baseline cho GAE.}
...
\centering
\begin{tikzpicture}[
  box/.style={draw, rounded corners=3pt, minimum width=2.75cm, minimum height=0.8cm, align=center, font=\scriptsize},
  >=stealth
]
\node[font=\small] (st)  at (0, 1.7)  {$s_t$};
\node[font=\small] (at)  at (0,-0.2)  {$a_t$};
\node[font=\small] (st1) at (0,-2.1)  {$s_{t+1}$};

\node[box, fill=gray!12] (enc1) at (2.5, 1.7)  {Encoder $\phi$};
\node[box, fill=gray!12] (enc2) at (2.5,-2.1)  {Encoder $\phi$};
\node[font=\tiny, gray] at (2.5, 0.75) {shared weights};

\node[box, fill=orange!14, minimum width=3.15cm] (inv) at (6.4, 2.25)
  {Inverse model\\$\hat{a}_t=g(\phi_t,\phi_{t+1})$};
\node[box, fill=blue!12, minimum width=3.15cm] (fwd) at (6.4,-0.2)
  {Forward model\\$\hat{\phi}_{t+1}=f(\phi_t,a_t)$};
\node[box, fill=green!14, minimum width=3.75cm] (rew) at (10.6,-0.2)
  {Intrinsic reward\\$r_t^{ICM}=\frac{1}{2}\|\hat{\phi}_{t+1}-\phi_{t+1}\|^2$};

\draw[->] (st) -- (enc1);
\draw[->] (st1) -- (enc2);

\draw[->] (enc1.east) -- node[above, font=\scriptsize]{$\phi_t$} (inv.west);
\draw[->] (enc1.east) -- ++(0.9,0) |- node[pos=0.25, right, font=\scriptsize]{$\phi_t$} (fwd.west);
\draw[->] (enc2.east) -- ++(0.7,0) |- node[pos=0.25, right, font=\scriptsize]{$\phi_{t+1}$} (inv.west);
\draw[->] (at.east) -- (fwd.west);
\draw[->] (fwd.east) -- node[above, font=\scriptsize]{$\hat{\phi}_{t+1}$} (rew.west);
\draw[->, dashed] (enc2.east) -- ++(3.6,0) -| node[pos=0.25, below, font=\scriptsize]{$\phi_{t+1}$} (rew.south);
\end{tikzpicture}
\caption{Kiến trúc ICM: hai Encoder $\phi$ chia sẻ trọng số để mã hóa $s_t$ và $s_{t+1}$ thành đặc trưng. Inverse model dự đoán hành động $\hat{a}_t$ từ hai đặc trưng; Forward model nhận $\phi(s_t)$ và $a_t$ để dự đoán $\hat{\phi}_{t+1}$; sai số tạo phần thưởng nội tại $r_t^{ICM}$.}
\label{fig:icm_arch}
\end{figure}
...
Trong đó $\sigma$ là hàm sigmoid, $[h_{t-1}, x_t]$ là phép ghép nối hai vectơ, và $W_f, W_i, W_c, W_o$ là các ma trận trọng số được học. Bốn đại lượng này lần lượt đóng vai trò: $f_t$ (forget gate) quyết định bao nhiêu thông tin cell cũ được giữ lại; $i_t$ (input gate) quyết định bao nhiêu thông tin mới được ghi vào; $\tilde{c}_t$ là giá trị cell ứng viên; $o_t$ (output gate) quyết định phần nào của cell trở thành đầu ra.
...
trong đó $C(i,j)$ là trạng thái của các ô lân cận $3\times 3$ xung quanh $(i,j)$ và $\mathbf{z} \in \mathbb{R}^k$ là nhiễu ngẫu nhiên được lấy mẫu độc lập mỗi lần sinh bản đồ. Đầu ra $t_{i,j} \in \{\text{wall},\, \text{floor}\}$ quyết định loại tile tại ô đó. Nhờ vectơ $\mathbf{z}$, cùng một genome $\theta$ có thể sinh ra nhiều bản đồ khác nhau mà không cần thay đổi trọng số mạng.

\textbf{Novelty search thay cho fitness trực tiếp.} Thay vì tối ưu trực tiếp chất lượng của từng bản đồ, PCGNN dùng \textit{novelty search}: hàm thích nghi của genome được đo bằng mức độ khác biệt của tập bản đồ mà genome đó sinh ra so với các genome khác trong quần thể. Điều này khuyến khích quần thể khám phá nhiều phong cách bản đồ thay vì hội tụ về một mẫu tối ưu duy nhất, tạo ra bộ sinh bản đồ đa dạng hơn cho curriculum.
...
Curriculum learning là phương pháp tổ chức quá trình học từ dễ đến khó \cite{Bengio2009}. Thay vì huấn luyện ngay trên phân phối khó nhất, tác nhân được bắt đầu từ các bản đồ dễ, sau đó chuyển dần sang Medium và Hard khi đạt ngưỡng hiệu quả nhất định. Cách tiếp cận này dựa trên quan sát rằng con người và động vật học hiệu quả hơn khi nhiệm vụ được sắp xếp từ đơn giản đến phức tạp, cho phép tích lũy kỹ năng nền trước khi đối mặt với thách thức cao hơn.

\textbf{Cơ chế chuyển bài học.} Trong Unity ML-Agents, điều kiện chuyển lesson được dựa trên phần thưởng trung bình trượt (exponential moving average). Tại mỗi bước $t$, phần thưởng được làm mượt bằng:

\begin{equation}
    \bar{r}_t = (1 - \alpha)\,\bar{r}_{t-1} + \alpha\, r_t,
    \label{eq:curriculum_ema}
\end{equation}

trong đó $\alpha \in (0,1)$ là hệ số làm mượt (thường đặt nhỏ để tránh nhảy lesson do nhiễu ngắn hạn). Khi $\bar{r}_t \geq \theta_\ell$ với $\theta_\ell$ là ngưỡng của lesson hiện tại $\ell$, trainer tự động tăng tham số môi trường \texttt{difficulty} lên, kích hoạt TilemapGenerator chuyển sang nhóm bản đồ khó hơn.

\textbf{Tại sao curriculum hiệu quả trong môi trường thưa thưởng?} Khi agent bắt đầu từ phân phối Hard với phần thưởng thưa, xác suất nhận được episode thành công (dọn phòng, tiêu diệt địch) rất thấp trong giai đoạn đầu. Gradient PPO lúc này gần như nhiễu ngẫu nhiên, khiến chính sách khó hội tụ. Curriculum giải quyết vấn đề này bằng cách tạo ra một ``giàn giáo gradient'': ở Easy, agent gặp thành công đủ thường xuyên để gradient có tín hiệu, từ đó học được chuỗi hành vi cơ bản trước khi vào môi trường khó hơn.

Trong đề tài, curriculum learning gắn trực tiếp với hệ thống PCGNN--TilemapGenerator: PCGNN tạo tập bản đồ ứng viên, bộ đánh giá chia bản đồ thành các nhóm Easy, Medium và Hard theo percentile, còn TilemapGenerator chọn map tương ứng với lesson hiện tại thông qua tham số \texttt{difficulty}. Đây cũng là yếu tố được đánh giá là có ảnh hưởng lớn nhất trong Chương~4, với mức cải thiện +85\% phần thưởng so với PPO baseline.

...
Run42 thêm LSTM với bộ nhớ kích thước 256 và chuỗi quan sát dài 64 bước. Khác với ICM/RND, LSTM có xu hướng học chậm nhưng tăng dần về cuối. Phần thưởng trung bình trượt 200k bước tăng từ 1.64 ở mốc 100k lên 2.71 ở mốc 1M. Điều này phù hợp với giả thuyết rằng bộ nhớ có ích cho hồi chiêu, đạn và trạng thái khuất tầm nhìn, nhưng lợi ích chưa đủ lớn để tạo thay đổi nổi bật trong quy mô huấn luyện hiện tại.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.94\textwidth}
        \small
        Hình cho thấy phần thưởng không tăng đều sau khi vào Hard mà dao động quanh một vùng ổn định. Run50 cao hơn run46 ở phần sau quá trình huấn luyện, gợi ý ICM chỉ phát huy rõ khi chính sách đã có kỹ năng nền từ curriculum.
    \end{minipage}
\end{figure}
...
Trong phạm vi các run đã thực hiện, đây là xu hướng mạnh nhất của khóa luận: curriculum không chỉ tăng phần thưởng trung bình mà còn tăng các chỉ số hành vi. Tail-50 của run46 đạt 0.994 kills/episode và tỷ lệ dọn phòng tail-50 đạt 0.267, cao hơn rõ rệt so với mốc so sánh. Về mặt cơ chế, curriculum learning đóng vai trò hỗ trợ tín hiệu học theo từng giai đoạn: ở Easy, tác nhân gặp các episode thành công đủ thường xuyên để học chuỗi tiếp cận -- ngắm -- tấn công; khi sang Medium và Hard, chính sách đã có kỹ năng nền thay vì khám phá từ gần như ngẫu nhiên. Tuy nhiên, do mỗi cấu hình mới chỉ có một seed, kết luận này cần được xác nhận thêm bằng các run lặp lại có error bar.

\section{Kết hợp curriculum với LSTM và ICM}

Run47 thêm LSTM vào curriculum. Trung bình ở Hard đạt 3.765, thấp hơn nhẹ so với 3.843 của run46. Chênh lệch này nhỏ và chưa đủ để kết luận LSTM làm xấu chính sách, nhưng đủ cho thấy bộ nhớ không tạo thêm cải thiện rõ ràng khi curriculum đã cung cấp khung đỡ cho khám phá.

Có hai giả thuyết giải thích kết quả này. Thứ nhất, \textit{LSTM làm chậm tốc độ hội tụ của PPO}: policy loss của run47 là 0.079, cao hơn gần 5 lần so với run46 là 0.017. Điều này gợi ý rằng các trọng số hồi tiếp của LSTM cần nhiều mẫu hơn để ổn định, khiến cập nhật chính sách kém hiệu quả hơn trong cùng 3M bước. Thứ hai, \textit{curriculum đã giải quyết phần lớn nhu cầu về bộ nhớ}: trong môi trường thưa thưởng, lợi ích chính của LSTM là giúp tác nhân nhớ chuỗi hành vi thành công trong episode dài. Tuy nhiên, khi curriculum đã thu hẹp phân phối bản đồ về Easy/Medium ở giai đoạn đầu, độ dài hiệu quả của chuỗi quyết định ngắn lại, làm giảm lợi thế tương đối của LSTM so với MLP thuần.

Run50 bổ sung ICM trên nền curriculum và được huấn luyện đủ 3.00M bước. Trên vùng Hard, phần thưởng trung bình đạt 4.369, cao hơn run46 là 3.843; tail-50 cuối run đạt 4.707. Một số chỉ số hành vi cũng cải thiện nhẹ, gồm số kẻ địch tiêu diệt trung bình đạt 1.016, tỷ lệ dọn phòng tail-50 đạt 0.273 và tỷ lệ dash hữu ích đạt 0.334. Điều này cho thấy ICM có thể hỗ trợ tác nhân khám phá thêm các hành vi có ích khi đã được curriculum dẫn qua giai đoạn học ban đầu. Tuy nhiên, inverse loss vẫn ở mức cao, khoảng 4.337, chỉ giảm nhẹ so với run35. Do đó, cải thiện của run50 chủ yếu thể hiện ở hành vi và phần thưởng, còn vấn đề biểu diễn phát sinh từ không gian hành động rời rạc nhiều nhánh chưa được ICM giải quyết triệt để.
...
Run55 thay đổi cấu trúc hàm thưởng: tăng mạnh phần thưởng tiêu diệt/dọn phòng/sát thương/tấn công đúng hướng, giảm phần thưởng tiếp cận và phạt mạnh hơn hành vi đứng gần địch không hành động. Run được huấn luyện đến 3.00M bước. Phần thưởng tail-50 đạt 9.600 và trung bình ở Hard đạt 9.337, nhưng không thể so trực tiếp với run46 vì thang thưởng đã thay đổi.

Chỉ số đáng chú ý hơn là đỉnh tỷ lệ dọn phòng đạt 0.80, cao hơn đỉnh 0.615 của run46. Điều này cho thấy reward v2 nhấn mạnh kết quả có thể đẩy tác nhân tấn công quyết đoán hơn. Tuy nhiên entropy tail vẫn quanh 4.408, nghĩa là reward v2 chưa phá được entropy plateau.
...
Trong phạm vi các ablation chính giữ nguyên quan sát 207 chiều, plateau này không được giải quyết bởi ICM, RND, LSTM, curriculum learning, giảm phương sai vị trí sinh hoặc thiết kế lại phần thưởng. Tuy nhiên, khi run62 kết hợp quan sát lưới cục bộ với curriculum 4 bậc, ICM và được huấn luyện đến 33.45M bước, entropy giảm bền vững xuống 3.10 nat và duy trì ở mức này. Kết luận thận trọng được cập nhật là: plateau này nhiều khả năng xuất phát từ biểu diễn quan sát 207 chiều và số bước huấn luyện 1--3M trong các ablation chính, chứ không phải từ giới hạn cứng của PPO hay action space hiện tại.
...
Kết quả của Bảng~\ref{tab:run59_bfs_assisted} cho thấy khi bổ sung tín hiệu đường đi ngắn nhất, các chỉ số hành vi tăng mạnh hơn: tỷ lệ dọn phòng đạt trên 0.8 ở vùng cuối, số kẻ địch bị tiêu diệt trung bình vượt 1.7 và entropy giảm sâu hơn so với plateau của các run 207 chiều. Đây là bước đột phá thực nghiệm lớn nhất trong toàn bộ quá trình thử nghiệm, vì nó không chỉ làm phần thưởng tăng mà còn cải thiện trực tiếp các chỉ số hành vi như tỷ lệ dọn phòng, số kẻ địch tiêu diệt và mức độ quyết đoán của chính sách. Điều này cho thấy nút thắt chính của môi trường không chỉ nằm ở thuật toán PPO hay cơ chế khám phá, mà còn nằm ở biểu diễn không gian của quan sát. Khi tác nhân được cung cấp đặc trưng đường đi phù hợp hơn, chính sách có thể học hành vi tiếp cận và chiến đấu ổn định hơn. Run59 vì vậy được xem là bước đột phá về biểu diễn quan sát, đồng thời gợi ý rằng các hướng tiếp theo nên tập trung vào local occupancy grid, graph/path feature học được, imitation learning cho điều hướng, hoặc thiết kế lại bản đồ/kẻ địch để giảm nhiễu từ các tình huống kẹt.

Do đó, run59 là bước cải thiện lớn nhất về hiệu năng thực nghiệm, nhưng bản chất của cải thiện này đến từ thay đổi biểu diễn quan sát chứ không chỉ từ thay đổi thuật toán học. Kết quả này không chứng minh tác nhân đã tự học tìm đường hoàn toàn, mà cho thấy khi được cung cấp đặc trưng đường đi phù hợp hơn, chính sách có thể học hành vi chiến đấu và tiếp cận mục tiêu ổn định hơn.

Bảng tổng hợp cho phép rút ra bốn nhận xét chính trong phạm vi thí nghiệm một seed. Thứ nhất, PPO baseline đạt hành vi hợp lý nhưng bị giới hạn ở mức phần thưởng khoảng 2.1--2.5. Thứ hai, intrinsic motivation và LSTM chưa tạo cải thiện lớn khi chạy độc lập. Thứ ba, curriculum learning là yếu tố nổi bật nhất; khi kết hợp thêm ICM, phần thưởng Hard tăng lên 4.369 nhưng inverse loss vẫn cao. Thứ tư, entropy plateau vẫn tồn tại trong nhóm ablation chính 207 chiều. Ở run62, khi kết hợp quan sát lưới cục bộ, curriculum 4 bậc, ICM và huấn luyện đến 33.45M bước, entropy giảm xuống 3.099 nat và clear\_rate đạt 0.626; tuy nhiên, do run62 thay đổi nhiều yếu tố đồng thời, kết quả này được xem là bằng chứng thăm dò thay vì ablation đơn biến.

\section{Observation bottleneck -- run62: local grid end-to-end}

Sau khi run59/61 cho thấy biểu diễn không gian và tín hiệu điều hướng là yếu tố đột phá, đề tài tiến hành run62 với cấu hình kết hợp: quan sát lưới cục bộ mở rộng đặc trưng không gian, curriculum 4 bậc (EasyFixed $\rightarrow$ MediumFixed $\rightarrow$ HardFixed $\rightarrow$ HardRandom), ICM với strength 0.001, và tham số \texttt{aggressive\_seek} luôn bật để kẻ địch truy đuổi tích cực hơn. Run này được thực hiện trên 8 môi trường song song trong hai session liên tiếp do giới hạn tài nguyên, tổng cộng đạt \textbf{33.45M bước} (24.25M bước trong session đầu, tiếp tục thêm 9.2M bước trong session sau với trọng số được khởi tạo từ checkpoint 24.25M).

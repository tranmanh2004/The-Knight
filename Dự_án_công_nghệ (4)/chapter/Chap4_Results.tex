...
Chương này trình bày kết quả thực nghiệm trên CombatAgent. Khác với kết quả ban đầu chỉ gồm một run ngắn 200 nghìn bước, bộ thí nghiệm cuối cùng gồm nhiều run từ 1 triệu đến 3 triệu bước, tập trung vào các yếu tố: phần thưởng nội tại, LSTM, curriculum learning, giảm phương sai vị trí sinh và thiết kế lại phần thưởng. Trong phạm vi các run đã thực hiện, curriculum learning là xu hướng cải thiện nổi bật nhất; ICM chỉ cho thấy xu hướng cải thiện rõ hơn khi được kết hợp với curriculum learning, còn các kỹ thuật còn lại chủ yếu dao động quanh cùng một mức trần hiệu năng.
...
\begin{table}[H]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}p{6.2cm}|>{\raggedright\arraybackslash}p{6.2cm}|}
        ...
        \hline
        Reward chính & reward\_tail50, reward\_hard\_from\_140k & Phần thưởng cuối run và phần thưởng trung bình sau khi curriculum đã vào Hard; dùng để so sánh hiệu năng thay vì chỉ nhìn peak. \\
        \hline
        Hành vi combat & kills\_tail50, cleared\_tail50, damage\_dealt\_tail50, damage\_taken\_tail50 & Cho biết tác nhân thật sự tiêu diệt địch, dọn phòng và trao đổi sát thương ra sao, tránh diễn giải phần thưởng tổng hợp một cách mơ hồ. \\
        \hline
        Dash & useful\_dash\_rate\_tail50, wasteful\_dash\_rate\_tail50, dash\_actions\_per\_decision\_tail50, blocked\_dash\_actions\_per\_decision\_tail50 & Đánh giá tác nhân có dùng dash như một kỹ năng chiến đấu hay gần như không dùng/không bị mask. \\
        \hline
        Policy & entropy\_tail50, extrinsic\_value\_estimate\_tail50 & Entropy đo mức ngẫu nhiên của policy; value estimate cho biết critic đang đặt kỳ vọng extrinsic reward ở mức nào. \\
        \hline
        Loss & policy\_loss\_tail50, value\_loss\_tail50, icm\_inverse\_loss\_tail50 & Theo dõi độ ổn định cập nhật PPO và chất lượng biểu diễn của ICM. \\
        \hline
        ...
        \hline
        Reward trung bình trượt & rolling reward 200k bước & Phần thưởng được làm mượt theo cửa sổ 200 nghìn bước, dùng để vẽ biểu đồ xu hướng học thay vì chỉ nhìn một điểm cuối. \\
        \hline
    \end{tabular}
\end{table}

Với các run curriculum, khóa luận ưu tiên reward mean trên vùng Hard từ sau bước 140 nghìn và các chỉ số hành vi ở tail-50 cuối run. Các cột peak, head/tail chi tiết hoặc mốc Hard phụ chỉ được dùng khi cần giải thích một hiện tượng cụ thể, vì bảng chính cần phản ánh năng lực chiến đấu và trạng thái học của policy hơn là ghi lại quá nhiều số log.
...
\section{PPO baseline và các phần thưởng nội tại}
...
Kết quả này cho thấy kiến trúc bộ nhớ có thể là hướng cải tiến phụ, nhưng không giải quyết vấn đề chính của môi trường: tác nhân vẫn bắt đầu từ phân phối Hard quá rộng và khó nhận được chuỗi phần thưởng thành công đủ thường xuyên.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.94\textwidth}
        \small
        Hình cho thấy phần thưởng không tăng đều sau khi vào Hard mà dao động quanh một vùng ổn định. Run50 cao hơn run46 ở phần sau quá trình huấn luyện, gợi ý ICM chỉ phát huy rõ khi policy đã có kỹ năng nền từ curriculum.
    \end{minipage}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.94\textwidth}
        \small
        Tỷ lệ dọn phòng tăng mạnh ở giai đoạn đầu rồi chững lại khi bài toán chuyển sang phân phối Hard. Vì clear rate là chỉ số kết quả, đường cong này bổ sung cho phần thưởng bằng cách cho thấy cải thiện không chỉ đến từ phần thưởng định hình.
    \end{minipage}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.94\textwidth}
        \small
        Entropy của ba run nhanh chóng rơi về vùng xấp xỉ 4.40 nat và gần như đi ngang. Điều này giải thích vì sao tăng phần thưởng/clear rate không đồng nghĩa với việc policy trở nên quyết định hơn đáng kể.
    \end{minipage}
\end{figure}
...
Trong phạm vi các run đã thực hiện, đây là xu hướng mạnh nhất của khóa luận: curriculum không chỉ tăng phần thưởng trung bình mà còn tăng các chỉ số hành vi. Tail-50 của run46 đạt 0.994 kills/episode và cleared tail-50 đạt 0.267, cao hơn rõ rệt so với baseline. Về mặt cơ chế, curriculum learning có thể được hiểu như một cơ chế hỗ trợ tín hiệu học theo từng giai đoạn: ở Easy, tác nhân gặp các episode thành công đủ thường xuyên để học chuỗi tiếp cận -- ngắm -- tấn công; khi sang Medium và Hard, policy đã có kỹ năng nền thay vì khám phá từ gần như ngẫu nhiên. Tuy nhiên, do mỗi cấu hình mới chỉ có một seed, kết luận này cần được xác nhận thêm bằng các run lặp lại có error bar.
...
Có hai giả thuyết giải thích kết quả này. Thứ nhất, \textit{LSTM làm chậm tốc độ hội tụ của PPO}: policy loss của run47 là 0.079, cao hơn gần 5 lần so với run46 là 0.017. Điều này gợi ý rằng các trọng số hồi tiếp của LSTM cần nhiều mẫu hơn để ổn định, khiến cập nhật policy kém hiệu quả hơn trong cùng 3M bước. Thứ hai, \textit{curriculum đã giải quyết phần lớn nhu cầu về bộ nhớ}: trong môi trường thưa thưởng, lợi ích chính của LSTM là giúp tác nhân nhớ chuỗi hành vi thành công trong episode dài. Tuy nhiên, khi curriculum đã thu hẹp phân phối bản đồ về Easy/Medium ở giai đoạn đầu, độ dài hiệu quả của chuỗi quyết định ngắn lại, làm giảm lợi thế tương đối của LSTM so với MLP thuần.

Run50 bổ sung ICM trên nền curriculum và được huấn luyện đủ 3.00M bước. Trên vùng Hard, reward mean đạt 4.369, cao hơn run46 là 3.843; tail-50 cuối run đạt 4.707. Một số chỉ số hành vi cũng cải thiện nhẹ, gồm số kẻ địch tiêu diệt trung bình đạt 1.016, cleared tail-50 đạt 0.273 và useful dash rate đạt 0.334. Điều này cho thấy ICM có thể hỗ trợ tác nhân khám phá thêm các hành vi có ích khi đã được curriculum dẫn qua giai đoạn học ban đầu. Tuy nhiên, inverse loss vẫn ở mức cao, khoảng 4.337, chỉ giảm nhẹ so với run35. Do đó, cải thiện của run50 chủ yếu thể hiện ở hành vi và phần thưởng, trong khi ICM chưa cho thấy khả năng giải quyết triệt để khó khăn biểu diễn phát sinh từ action space rời rạc nhiều nhánh.
...
Kết quả cập nhật này làm kết luận tinh tế hơn: curriculum vẫn là yếu tố nền quan trọng nhất, nhưng ICM có thể tạo thêm lợi ích khi tác nhân đã có kỹ năng nền từ các lesson dễ hơn. Dù vậy, inverse loss còn cao cho thấy curiosity signal chủ yếu hỗ trợ khám phá/hành vi ở mức thực nghiệm, chưa chứng minh rằng representation của ICM đã học tốt quan hệ hành động--chuyển trạng thái trong môi trường này.

\section{Giảm phương sai môi trường bằng vị trí sinh cố định rồi ngẫu nhiên}

Run54 mở rộng curriculum thành 4 lesson: Easy Fixed, Medium Fixed, Hard Fixed và Hard Random. Lesson Hard Random bắt đầu khoảng 280k bước. Reward Hard mean đạt 3.925, số lần tiêu diệt cuối run đạt 0.945 và sát thương gây ra đạt 33.673. Các chỉ số này gần run46 nhưng không vượt rõ, nên kết quả này được xem là tương đương với mức cải thiện nhỏ.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Curriculum 4-tier của run54: vị trí sinh cố định trước, vị trí sinh ngẫu nhiên sau}
...
Run55 thay đổi cấu trúc hàm thưởng: tăng mạnh phần thưởng tiêu diệt/dọn phòng/sát thương/tấn công đúng hướng, giảm phần thưởng tiếp cận và phạt đứng gần địch không hành động mạnh hơn. Log cuối cùng đã đạt 3.00M bước. Reward tail-50 đạt 9.600 và Hard mean đạt 9.337, nhưng không thể so trực tiếp với run46 vì thang thưởng đã thay đổi.

Chỉ số đáng chú ý hơn là cleared peak đạt 0.80, cao hơn peak 0.615 của run46. Điều này cho thấy reward v2 nhấn mạnh kết quả có thể đẩy tác nhân tấn công quyết đoán hơn. Tuy nhiên entropy tail vẫn quanh 4.408, nghĩa là reward v2 chưa phá được entropy plateau.

Do đó, run55 được tách khỏi nhóm so sánh reward v1 khi diễn giải reward mean. Với reward v2, các chỉ số đáng dùng hơn là cleared rate, kills, sát thương gây ra, sát thương nhận vào, survival hoặc các metric đã chuẩn hóa theo cùng thang đo. Cách tách này tránh việc hiểu nhầm rằng giá trị phần thưởng 9.337 của run55 có thể so trực tiếp với giá trị 3.843 của run46.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Run55 tăng peak cleared rate, nhưng reward mean không so trực tiếp do đổi thang thưởng}
...
Trong phạm vi các ablation chính giữ nguyên observation 207 chiều, plateau này không được giải quyết bởi ICM, RND, LSTM, curriculum learning, giảm phương sai vị trí sinh hoặc thiết kế lại phần thưởng. Kết luận thận trọng là: entropy plateau là giới hạn thực nghiệm quan sát được trong thiết kế action space và môi trường hiện tại, chưa phải một định lý về PPO. Hướng tiếp theo hợp lý là đơn giản hóa action space, gom bớt nhánh hoặc thêm inductive bias cho ngắm và né đạn.
...
\end{tikzpicture}
\caption{So sánh reward mean giữa các run dùng reward v1. Đường đứt khoảng ở 2.07 là mức baseline. Run55 (reward v2, mean 9.337) có thang thưởng khác nên không đưa vào biểu đồ.}
...
\begin{figure}[H]
    ...
    \caption{So sánh các chỉ số hành vi cuối run. Hình này bổ sung cho Bảng~\ref{tab:run_metrics_main} bằng cách cho thấy curriculum không chỉ tăng phần thưởng mà còn cải thiện số kẻ địch bị tiêu diệt, tỷ lệ dọn phòng và tỷ lệ dash hữu ích.}
    \label{fig:final_metrics_comparison}
\end{figure}

\section{Ablation mở rộng: quan sát hỗ trợ BFS}

Sau các run ablation chính về kỹ thuật huấn luyện, đề tài thực hiện thêm run59 như một ablation mở rộng về biểu diễn quan sát. Mục tiêu của run này là kiểm tra một giả thuyết cụ thể: tác nhân có thể không thất bại chủ yếu vì không biết chiến đấu, mà vì biểu diễn quan sát 207 chiều chưa cung cấp đủ cấu trúc không gian để policy suy luận đường đi qua bản đồ có tường. Để kiểm tra giả thuyết này, run59 bổ sung ba chiều quan sát vào CombatAgent: hướng bước kế tiếp theo đường đi ngắn nhất tính bằng BFS và khoảng cách đường đi ngắn nhất đã chuẩn hóa tới kẻ địch còn sống gần nhất. Do đó, vector quan sát tăng từ 207 lên 210 chiều.

Điểm quan trọng khi diễn giải run59 là cấu hình này bổ sung yếu tố pathfinding vào quá trình học, nên không được đặt ngang hàng tuyệt đối với các run giữ nguyên observation 207 chiều. Chính sách vẫn được huấn luyện bằng PPO và vẫn phải học lựa chọn hành động di chuyển, ngắm, tấn công và dash, nhưng phần điều hướng nhận thêm tín hiệu hỗ trợ từ BFS. Nói cách khác, đây là biến thể \emph{BFS-assisted RL} hoặc \emph{path-informed observation}, dùng để đo tác động của tín hiệu đường đi đối với hiệu năng của tác nhân.

\begin{table}[H]
    \centering
    \small
    \caption{Kết quả cấu hình path-informed sau bản cập nhật run61 ở mốc 1.0M bước}
    ...
    \begin{tabular}{|p{4.1cm}|p{3.2cm}|p{6.4cm}|}
        ...
        \hline
        Cumulative reward & 14.09 cuối run; tail gần cuối khoảng 14.77 & Cao hơn rõ so với các run chính, nhưng không so trực tiếp vì observation và reward shaping đã thay đổi. \\
        \hline
        Cleared-room rate & 0.727 cuối run; tail gần cuối khoảng 0.846 & Tác nhân dọn phòng ổn định hơn khi được cung cấp tín hiệu đường đi. \\
        \hline
        Episode kills & 1.55 cuối run; tail gần cuối khoảng 1.78 & Số kẻ địch bị tiêu diệt tăng, cho thấy khả năng điều hướng là một nút thắt thực tế của bài toán. \\
        \hline
        Policy entropy & Giảm từ khoảng 4.15 xuống 3.59; tail gần cuối khoảng 3.62 & Policy trở nên quyết đoán hơn so với các run bị plateau quanh 4.40 nat. \\
        \hline
        Curiosity reward & Khoảng 0.005 ở cuối run & ICM không còn chi phối hành vi ở cuối run. \\
        \hline
        Enemy stuck recoveries & Tail gần cuối khoảng 3.07 mỗi episode & Môi trường vẫn còn nhiễu do kẻ địch bị kẹt hoặc cần recovery, nên kết quả cần được đọc kèm hạn chế runtime. \\
        \hline
    \end{tabular}
\end{table}

Kết quả của Bảng~\ref{tab:run59_bfs_assisted} cho thấy khi bổ sung tín hiệu đường đi ngắn nhất, các chỉ số hành vi tăng mạnh hơn: tỷ lệ dọn phòng đạt trên 0.8 ở vùng cuối, số kẻ địch bị tiêu diệt trung bình vượt 1.7 và entropy giảm sâu hơn so với plateau của các run 207 chiều. Đây là bước đột phá thực nghiệm lớn nhất trong toàn bộ quá trình thử nghiệm, vì nó không chỉ làm phần thưởng tăng mà còn cải thiện trực tiếp các chỉ số hành vi như clear rate, kills và mức độ quyết đoán của policy. Điều này cho thấy nút thắt chính của môi trường không chỉ nằm ở thuật toán PPO hay cơ chế exploration, mà còn nằm ở biểu diễn không gian của observation. Khi tác nhân được cung cấp đặc trưng đường đi phù hợp hơn, policy có thể học hành vi tiếp cận và chiến đấu ổn định hơn. Run59 vì vậy được xem là bước đột phá về biểu diễn quan sát, đồng thời gợi ý rằng các hướng tiếp theo nên tập trung vào local occupancy grid, graph/path feature học được, imitation learning cho điều hướng, hoặc thiết kế lại bản đồ/kẻ địch để giảm nhiễu từ các tình huống kẹt.

Do đó, run59 là bước cải thiện lớn nhất về hiệu năng thực nghiệm, nhưng bản chất của cải thiện này đến từ thay đổi biểu diễn quan sát chứ không chỉ từ thay đổi thuật toán học. Kết quả này không chứng minh tác nhân đã tự học pathfinding hoàn toàn, mà cho thấy khi được cung cấp đặc trưng đường đi phù hợp hơn, policy có thể học hành vi chiến đấu và tiếp cận mục tiêu ổn định hơn.

Từ bảng tổng hợp, bốn nhận xét chính được rút ra trong phạm vi thí nghiệm một seed. Thứ nhất, PPO baseline đạt hành vi hợp lý nhưng bị giới hạn ở mức phần thưởng khoảng 2.1--2.5. Thứ hai, intrinsic motivation và LSTM chưa tạo cải thiện lớn khi chạy độc lập. Thứ ba, curriculum learning là yếu tố nổi bật nhất; khi kết hợp thêm ICM, phần thưởng Hard tăng lên 4.369 nhưng inverse loss vẫn cao. Thứ tư, entropy plateau vẫn tồn tại trong nhóm ablation chính 207 chiều, cho thấy cần nghiên cứu thêm về action space và cấu trúc policy.

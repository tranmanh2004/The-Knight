...
Trong phạm vi các ablation đã thử, Trong phạm vi các ablation đã thử, plateau này không được giải quyết bởi ICM, RND, LSTM, curriculum learning, giảm phương sai spawn hoặc reward redesign. Kết luận thận trọng là: entropy plateau là giới hạn thực nghiệm quan sát được trong thiết kế action space và môi trường hiện tại, chưa phải một định lý về PPO. Hướng tiếp theo hợp lý là đơn giản hóa action space, gom bớt branch hoặc thêm inductive bias cho ngắm và né đạn.
...
\section{Thí nghiệm chẩn đoán với quan sát hỗ trợ BFS}

Sau các run ablation chính, đề tài thực hiện thêm run59 nhằm kiểm tra một giả thuyết cụ thể: agent có thể không thất bại chủ yếu vì không biết chiến đấu, mà vì không suy luận được đường đi qua bản đồ có tường chỉ từ vị trí tương đối của enemy và các wall ray. Để kiểm tra giả thuyết này, run59 bổ sung ba chiều quan sát vào CombatAgent: hướng bước kế tiếp theo đường đi ngắn nhất tính bằng BFS và khoảng cách đường đi ngắn nhất đã chuẩn hóa tới kẻ địch còn sống gần nhất. Do đó, vector quan sát tăng từ 207 lên 210 chiều.

Điểm quan trọng là run59 không được xem là một baseline RL thuần. Chính sách vẫn được huấn luyện bằng PPO và vẫn phải học lựa chọn hành động di chuyển, ngắm, tấn công và dash, nhưng phần navigation đã nhận thêm thông tin hỗ trợ từ môi trường. Nói cách khác, đây là biến thể \emph{BFS-assisted RL} hoặc \emph{path-informed observation}, không phải cấu hình mà agent tự học toàn bộ pathfinding từ quan sát cục bộ.

\begin{table}[H]
    \centering
    \small
    \caption{Kết quả run59 với quan sát hỗ trợ BFS ở mốc 5.0M bước}
    ...
    \begin{tabular}{|p{4.1cm}|p{3.2cm}|p{6.4cm}|}
        ...
        \hline
        Cumulative reward & 16.66; tail gần cuối khoảng 16.37 & Cao hơn rõ so với các run chính, nhưng không so trực tiếp vì observation và reward shaping đã thay đổi. \\
        \hline
        Cleared-room rate & 0.818 cuối run; tail gần cuối khoảng 0.712 & Agent dọn phòng ổn định hơn khi được cung cấp tín hiệu đường đi. \\
        \hline
        Episode kills & 1.79 cuối run; tail gần cuối khoảng 1.60 & Số enemy bị tiêu diệt tăng, cho thấy navigation là một nút thắt thực tế của bài toán. \\
        \hline
        Policy entropy & Giảm từ khoảng 4.47 xuống 3.70 & Policy trở nên quyết đoán hơn so với các run bị plateau quanh 4.40 nat. \\
        \hline
        Curiosity reward & Giảm xuống khoảng 0.006 & ICM không còn chi phối hành vi ở cuối run. \\
        \hline
        Enemy stuck recoveries & Khoảng 2.94 mỗi episode & Môi trường vẫn còn nhiễu do enemy bị kẹt hoặc cần recovery, nên kết quả cần được đọc kèm hạn chế runtime. \\
        \hline
    \end{tabular}
\end{table}

Kết quả của Bảng~\ref{tab:run59_bfs_assisted} cho thấy khi agent được nhìn thấy hướng đường đi ngắn nhất, các chỉ số hành vi tăng mạnh hơn: tỷ lệ dọn phòng đạt trên 0.7 ở vùng cuối, số enemy bị tiêu diệt trung bình vượt 1.5 và entropy giảm sâu hơn so với plateau của các run 207 chiều. Điều này ủng hộ giả thuyết rằng navigation qua bản đồ procedural là một khó khăn lớn của môi trường hiện tại. Tuy nhiên, kết quả này không nên được dùng để khẳng định rằng agent đã tự học pathfinding. Run59 có giá trị chính như một thí nghiệm chẩn đoán: nếu cung cấp thông tin đường đi làm hiệu năng tăng rõ, thì các hướng tiếp theo nên tập trung vào biểu diễn không gian tốt hơn như local occupancy grid, graph/path feature học được, imitation learning cho navigation, hoặc thiết kế lại map/enemy để giảm nhiễu từ các tình huống kẹt.
